...
extern crate stable_heap as heap;

use std::{cmp, marker, mem, ptr};
    ...
    unsafe fn new_uninitialized(capacity: usize) -> RawTable<K, V> {
        if capacity == 0 {
            return RawTable {
                size: 0,
                capacity: 0,
                hashes: Unique::new(heap::EMPTY as *mut u64),
        ...
        let buffer = heap::allocate(size, malloc_alignment);
...
impl<K, V> Drop for RawTable<K, V> {
    fn drop(&mut self) {
        ...
        unsafe {
            heap::deallocate(*self.hashes as *mut u8, size, align);
